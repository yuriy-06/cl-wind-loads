...
(defmethod wind-plosk-opora ((obj1 plosk-opora))   ;;надо добавить ветер от распорки
  (let ((i 0) m m2 m3 sin-v l-sv wx wy tan-v alfa-v)
    (dolist (x (z-list obj1))
      (progn (push (- x i) m)
	     (setf i x)))
    (setf (dh obj1) (reverse m))
    (setf m (dh obj1))
    (dolist (x m)
    ...
    (dotimes (i (length (z-list obj1))) 
      (progn (setf wx (ocrugl-1.000 (wind-x :h (nth i (dh obj1)) :sinx (nth i (sin-list obj1)) 
					    :h-k (h (kolonna obj1)) :h-sv (h (svayz* obj1)) :k (nth i (k-list obj1))  :c *c*))
		   wy (ocrugl-1.000 (wind-y :h (nth i (dh obj1)) :l-sv (nth i (l-sv-list obj1)) 
